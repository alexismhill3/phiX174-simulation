...
basedir <- "/stor/work/Wilke/tingle/phix174/output/3152022/"
...
Here I will analyze the results from the promoter and terminator optimization run on 3-15-2022. These results are form the new optimization algorithm that differs from previous algorithms in several distinct ways: 

1. In addition to the three terminators (pA, pB, pD), the four terminators (tJ, tF, tG, tH) are also being optimized. 

2. Terminator optimization is additive (+/- step of 0.05)

3. Promoter optimization still multiplicative ( [randomly select step size + 1] * multiply existing exponent value).
  * Ex Step 'back':  pA = e^3; randomly select -0.4 --> 0.6 scale; so pA = e^(3*0.6)
  * Ex Step 'forward: pA = e^3; randomly select +0.4 --> 1.4 scale; so pA = e^(3*1.4)

### **Change in Error over Generations**
These results look the best out of all the previous optimizations!! 
```{r, echo = FALSE, message = FALSE, warning=FALSE}
filedir <- ""
plot_reports(basedir, filedir, 10, "qPCR Simulations")
```
...
all_sims_report <- get_reports(basedir, filedir, 10) %>% 
  group_by(sim) %>% 
...
```{r, echo = FALSE, message = FALSE, warning=FALSE}
sim9 <- logel_plot(basedir, filedir, 9, 396, 1200) + ggtitle("Sim 9")
sim7 <- logel_plot(basedir, filedir, 7, 260, 1200) +  ggtitle("Sim 7")
sim3 <- logel_plot(basedir, filedir, 3, 55, 1200) + ggtitle("Sim 3")
sim4 <- logel_plot(basedir, filedir, 4, 318, 1200) + ggtitle("Sim 4")

plot_grid(sim9, sim7, sim3, sim4, nrow = 2, ncol = 2)
```
...
pt_run <- as.data.frame(read_tsv(file = "/stor/work/Wilke/tingle/phix174/output/3152022/sim_9_gen_369.tsv")) %>% 

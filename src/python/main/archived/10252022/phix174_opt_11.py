...
def calculate_fitness(error, beta, beta_start, beta_end, gen, max_gen, fitness):
    if (gen > 500):
      beta = beta + ((beta_end - beta_start)/(max_gen - 500))
    new_fitness = 1/(np.exp(beta * error) + 1) # equation 3 from Shah et al.
    
    if (new_fitness > fitness):
      return (new_fitness, beta, True)
    else:
      return (fitness, beta, False)

# STEP 4:
def main(u, o, date, sim, base_dir, beta_start, beta_end, max_gen):
    gen = 0
    ...
    while (gen < max_gen):
        ...
        while (gen == 0):
            try:
                ...
                if (new_error == -1):
                    ...
                    print(f"\n")
                    print(gen, pA, pB, pD, tJ, tF, tG, tH)
                    fitness, beta, _ = calculate_fitness(new_error, beta, beta_start, beta_end, 0, max_gen, 0)
                    new_run = {'gen': gen, 'pA': pA, 'pB': pB, 'pD': pD, 'tJ': tJ, 'tF': tF, 'tG': tG, 'tH': tH,
                               'error': new_error, 'min_error': new_error, 'fitness': fitness}
        ...
        new_error = get_error(file=base_dir + "output/" + str(date) + "/sim_" + str(sim) + "_gen_" + str(gen) + ".tsv", gen=gen)
        fitness, beta, accept = calculate_fitness(new_error, beta, beta_start, beta_end, gen, max_gen, fitness)
        print(f"Fitness: {fitness}, Beta: {beta}, Accept: {accept}")

        if (not accept):
            # Save poor run
            new_run = {'gen': gen, 'pA': pA, 'pB': pB, 'pD': pD, 'tJ': tJ, 'tF': tF, 'tG': tG, 'tH': tH,
                       'error': new_error, 'min_error': min_error, 'fitness': fitness}
            report_df = report_df.append(new_run, ignore_index=True)

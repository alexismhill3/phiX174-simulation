...
def run_pt(sim, ID, pA, pB, pD, tJ, tF, tG, tH, base_dir, output_dir):
    ...
    phage = pt.Genome(name="phix_174", length=5420)
    ...
    phage.add_terminator(name="terminator_H", start=5415, stop=5416, #move terminator to end, before pA
                 efficiency={"ecolipol": tH})
    ...
    model.simulate(time_limit=500, time_step=5,output=output_dir + "/circular_" + sim + "_ID_" + str(ID) + ".tsv")
...
if __name__ == "__main__":
    #sim = "tH_pA" #with original fitted parameters
    sim = "tH_pA_0.5"
    base_dir = sys.argv[1]

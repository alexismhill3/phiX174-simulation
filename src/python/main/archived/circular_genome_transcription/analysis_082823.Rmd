...
date <- "082823"
filedir <- paste("/stor/scratch/Wilke/amh7958/phix174-simulation/src/python/main/", "circular_genome_transcription", "/", date,  sep="")
figdir <- paste("/stor/scratch/Wilke/amh7958/phix174-simulation/src/python/main/", "circular_genome_transcription", "/", "figures/", sep="")
...
swap <- logel_plot_2(sim_swap) + ggtitle("tH-pA, pA = 8.44")
...
swap_1 <- logel_plot_2(sim_swap_1) + ggtitle("tH-pA, pA = 0.5")
```
```{r}
...
decomp <- left_join(sim_wt_prot_only, sim_decomp_prot_only) %>%
  mutate(fold_change = (protein_decomp - protein_wt) / protein_wt) %>%
...
comp %>% ggplot(aes(experiment, fold_change, fill=experiment)) +
  geom_col() +
  facet_wrap(~species) +

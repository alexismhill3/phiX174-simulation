...
#### Summary
Copy of the original analysis file from this folder, for messing around with plots for my proposal.
...
logel_plot <- function(filedir, sim, gen, timestamp, label){
...
logel_timeseries_with_pcr <- function(filedir, sim, gen1, gen2, gen3, timestamp) {
  ...
  sim_qpcr <- data.frame(species = c("A", "A*", "B", "K","C", "D", "E", "J","F", "G", "H"),
            ...
            facet_wrap(~title) +
            scale_fill_manual(values = my_palette) + 
            scale_y_continuous(limits = c(0, 300), expand = c(0,0), breaks = c(100, 200)) +
            ylab("Transcripts")+ 
            xlab("ΦX174 genes")+ 
            theme_bw() +
            theme(legend.position = "none",
  ...
  sim_start <- logel_plot(filedir, sim, gen = gen1, timestamp = timestamp, label = gen1)
              
  sim_middle <- logel_plot(filedir, sim, gen = gen2, timestamp = timestamp, label = gen2) 
  
  sim_end <- logel_plot(filedir, sim, gen = gen3, timestamp = timestamp, label = gen3)
...
```{r}
logel_timeseries_with_pcr(filedir, sim_num, "0", "415", "3310", time_stamp)
ggsave("grant_figures/timeseries_sim_18_short.png", width = 9, height = 3)
```
...
promoter_dist_2 <- best_gen_report %>% 
  ...
  pivot_longer(!sim, names_to = "element", values_to = "strength") %>%
  filter(element %in% c("pA", "pB", "pD")) %>%
  ggplot(aes(x=fct_relevel(element, "pD", "pB", "pA"), y=strength)) +
...
terminator_dist_2 <- best_gen_report %>% 
  ...
  pivot_longer(!sim, names_to = "element", values_to = "strength") %>%
  filter(!element %in% c("pA", "pB", "pD")) %>%
  ggplot(aes(x=fct_relevel(element, "tH", "tG", "tF", "tJ"), y=strength)) +
    geom_boxplot(fill = "gray67") +
    scale_y_continuous(limits = c(0, 1)) +
    scale_fill_manual(values = term_palette) +
    ylab("termination probability") +
...
estimates <- cowplot::plot_grid(promoter_dist_2, terminator_dist_2, pa_ph_corr,
                      ...
                      labels = c("B", "", "C"), 
                      ncol = 3, nrow = 1)
cowplot::plot_grid(logel, estimates, ncol = 1, nrow = 2, labels = c("A", ""))
```

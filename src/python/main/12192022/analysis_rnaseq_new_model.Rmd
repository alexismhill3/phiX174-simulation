...
```{r}
# plot RMSE curves for each simulation (requires raw data)
# plot_reports(filedir, "RNAseq")
```

```{r}
#best_gen_report <- best_gen(filedir)
#write_csv(best_gen_report, "best_gen_report_putative_rnaseq.csv")
```

### If raw data is not available, start here

```{r}
best_gen_report <- read_csv("best_gen_report_putative_rnaseq.csv")
best_gen_report
```


```{r}
local_fildir <- "sim_output/" # simulation files for plotting have been copied here
logel_timeseries_with_rnaseq(local_fildir, "7", "0", "500", "3878", 500)
...
best_gen_report_original_model
```
...
term_all %>%
  ggplot(aes(x=element, y=strength, fill=model)) +
    ...
    panel_border()
    #theme(legend.position = "none")
#ggsave("figures/term_estimates.png", width = 9, height = 3)
```
```{r}
prom_all %>%
  ggplot(aes(x=element, y=strength, fill=model)) +
...
for(i in 0:4){
     tmp <- read_tsv(paste0(local_fildir, "/sim_7_gen_3878_ID_", i, ".tsv")) %>%
       mutate(time = round(time)) %>% 
       filter(time == 500)
...
#fildir_canonical <- paste("/stor/scratch/Wilke/amh7958/phix174-simulation/output/", "01222023", "/", sep="")
df_canonical <- data.frame()
for(i in 0:4){
     tmp <- read_tsv(paste0(local_fildir, "/sim_15_gen_2467_ID_", i, ".tsv")) %>%
       mutate(time = round(time)) %>% 
       filter(time == 500)
...
```
```{r}
row <- cowplot::plot_grid(prom + theme(legend.position = "none"), term, nrow = 1, rel_widths = c(0.45, 0.65))
